...
Describe 'Export-AuditHtml' {
    It 'writes valid HTML file with table' {
        ...
        # Severity is conveyed by glyph, not color CSS.
        $content | Should -Match 'class="glyph"'
        $content | Should -Match ([char]0x2717)  # Error glyph
        $content | Should -Match ([char]0x2713)  # Healthy glyph
        $content | Should -Not -Match 'class="error"'
        $content | Should -Not -Match 'class="healthy"'
    }

    It 'HTML-encodes hostile cell values' {
        $dt = New-Object System.Data.DataTable
        [void]$dt.Columns.Add("Name", [string])
        [void]$dt.Columns.Add("Status", [string])
        [void]$dt.Rows.Add('<script>alert(1)</script>', 'Healthy')
        [void]$dt.Rows.Add('A & B "C"', 'Healthy')

        $htmlPath = Join-Path $TestDrive 'hostile.html'
        Initialize-Logging -LogPath (Join-Path $TestDrive 'hostile.log')
        Export-AuditHtml -DataTable $dt -OutputPath $htmlPath -ReportTitle '<b>Pwn</b>'
