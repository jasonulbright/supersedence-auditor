...
function Initialize-Logging {
    param(
        [string]$LogPath,
        # Bind the path on this module instance only; do NOT (re)write the
        # header. Used by background runspaces to attach to the UI's existing
        # log file without clobbering already-written lines via Set-Content.
        [switch]$Attach
    )
...
function Export-AuditHtml {
    <#
    .SYNOPSIS
        Exports a DataTable to a self-contained HTML report. Status is conveyed
        via glyph (check, x, warn, ellipsis) at theme foreground; no colored
        severity fills (per brand: hue-coded status fails WCAG SC 1.4.1 and is not in
    ...
    $css = @(
        '<style>',
        'body { font-family: "Segoe UI", Arial, sans-serif; margin: 20px; background: #fafafa; color: #252525; }',
        'h1 { color: #0078D4; margin-bottom: 4px; }',
        '.summary { color: #595959; margin-bottom: 12px; font-size: 0.9em; }',
        'table { border-collapse: collapse; width: 100%; margin-top: 12px; }',
        'th { background: #0078D4; color: #fff; padding: 8px 12px; text-align: left; }',
        'td { padding: 6px 12px; border-bottom: 1px solid #e0e0e0; }',
        'tr:nth-child(even) { background: #f5f5f5; }',
        '.glyph { font-family: "Segoe UI Symbol", "Segoe UI", Arial, sans-serif; display: inline-block; width: 1.4em; }',
        '</style>'
    ) -join "`r`n"
    ...
    $glyphMap = @{
        ...
        'Info'            = [char]0x22EF
        'Healthy'         = [char]0x2713
        'Orphaned'        = [char]0x2717
        'Circular'        = [char]0x2717
        'Missing Content' = [char]0x2717
        'Expired Target'  = [char]0x26A0
        'Disabled Source' = [char]0x26A0
        'Disabled Target' = [char]0x26A0
        'Undocumented'    = [char]0x22EF
    }

    # HTML-encode every interpolated value. App names, descriptions, and
    # remediation text can contain <, >, &, or quote characters that would
    # otherwise produce malformed reports or inject markup.
    $enc = { param([string]$s) [System.Net.WebUtility]::HtmlEncode($s) }

    $headerRow = ($DataTable.Columns | ForEach-Object { "<th>$(& $enc $_.ColumnName)</th>" }) -join ''
    $bodyRows = foreach ($row in $DataTable.Rows) {
        $cells = foreach ($col in $DataTable.Columns) {
            $val = [string]$row[$col.ColumnName]
            if (($col.ColumnName -eq 'Severity' -or $col.ColumnName -eq 'Status') -and $glyphMap.ContainsKey($val)) {
                $g = $glyphMap[$val]
                "<td><span class=`"glyph`">$g</span>$(& $enc $val)</td>"
            } else {
                "<td>$(& $enc $val)</td>"
            }
        }
